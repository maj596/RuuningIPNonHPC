...
def main(argv=None):
    ...
    # Loss function
    loss = lossfunc.jaccard_loss(y_,y) #crossentropy seems to be giving 0 dice loss so maybe try something else

    #Train initialize author gives error nan in tensor a100 but not in v100 (0 dice loss here though)
    # trainable_var = tf.trainable_variables()
    # optimizer = tf.train.AdamOptimizer(opt.lr)
    # grads = optimizer.compute_gradients(loss, trainable_var)
    # train_op = optimizer.apply_gradients(grads)
    
    # Train initialize works but i like others
    trainable_var = tf.trainable_variables()
    optimizer = tf.train.RMSPropOptimizer(opt.lr)
    grads = optimizer.compute_gradients(loss, trainable_var)
    train_op = optimizer.minimize(loss)
    
    # Train initialize works
    trainable_var = tf.trainable_variables()
    optimizer = tf.train.GradientDescentOptimizer(learning_rate=opt.lr)
    grads = optimizer.compute_gradients(loss, trainable_var)
    train_op = optimizer.apply_gradients(grads)
    
    
    # Train initialize  my adam version
    # trainable_var = tf.trainable_variables()
    ...
    # # You can add any additional modifications to the gradients here
    # # For example, you can clip the gradients to prevent them from exploding
    ...
    # lr = tf.train.exponential_decay(opt.lr, global_step, decay_steps=100, decay_rate=0.9)
    # trainable_var = tf.trainable_variables()
    ...
    for itr in xrange(o_itr,opt.max_iteration):
        ...
        # print("cat",train_pred)
        train_annotations = np.squeeze(train_annotations,axis=1)
        train_annotations = np.squeeze(train_annotations,axis=-1)
        for i in range(opt.batch_size):
            # returned = confusion_matrix(train_annotations[i].argmax(axis=1),train_pred[i].argmax(axis=1)).ravel() #LOOOK ARHAM I MADE THIS CHANGE
        # for var in returned:
        #     print (len(var))


            train_Dice_sum += utils.cal_Dice(train_pred[i],train_annotations[i])
            # train_Dice_sum += utils.cal_mse(train_pred[i],train_annotations[i])
            ...
            # print("ice",train_pred[i])
            # print("dog",train_annotations[i])
        ...
        if itr % opt.print_info_freq == 0:
            print("Step:{}, Train_loss:{}, Train_acc:{}, Train_Dice:{}".format(itr,round(train_loss_sum/opt.print_info_freq,4),   \
                                                                               ...
                                                                               train_Dice_sum/opt.print_info_freq/opt.batch_size )) #gmean, kappa, auc, fdr gmean:{} kappa:{} auc:{} fdr:{}
            train_loss_sum = 0.0
            train_acc_sum = 0.0
            train_Dice_sum = 0.0
            ...
            if val_num > temp_index * opt.batch_size:
                ...
                valid_pred = np.squeeze(valid_pred[0],axis=1)
                valid_annotations = np.squeeze(valid_annotations,axis=1)
                # valid_annotations = np.squeeze(valid_annotations,laxis=-1)
                temp_index += 1
